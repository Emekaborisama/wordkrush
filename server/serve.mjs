 ...
 * of files. This is the ~150 lines that join the two, with no dependency added
 * to the app (agents.md: prefer platform APIs where sufficient).
 *
 * It is deliberately not just "read file, write response". Three things about
 * this particular payload make the difference between a fast game and a slow
 * one:
 *
...
// Railway routes to the container's external interface; binding to localhost
// would make the service unreachable while looking perfectly healthy in logs.
const HOST = '0.0.0.0';
...
/** Below this, framing overhead cancels out any saving. */
const MIN_COMPRESS_BYTES = 1024;

/** Absolute path -> the everything-we-need-to-serve-it record. */
const files = new Map();
...
/**
 * Cache policy, decided by where the file sits.
 *
 * Expo puts content-hashed filenames under `_expo/`, so those are immutable by
 * construction: a change produces a different URL. Everything else keeps a
 * short leash, and HTML gets none at all.
 */
...
async function warm() {
  ...
  for (const absolute of paths) {
    ...
    const record = {
      ...
      // Content-derived, so a rebuilt-but-identical file keeps its ETag and
      // returning players still get a 304 rather than a fresh download.
    ...
    if (COMPRESSIBLE.test(type) && body.length >= MIN_COMPRESS_BYTES) {
      ...
      // Only keep a compressed form that actually won.
...
/**
 * Resolves a request path to a served file.
 *
 * `posix.normalize` on a leading-slash path collapses any `..` before it can
 * escape, so a request for `/../../etc/passwd` resolves to `/etc/passwd` and
 * then simply misses the map — there is no filesystem lookup to traverse.
 */
function lookup(rawUrl) {
  ...
  // Single-page app fallback. Only for paths that look like routes: answering a
  // missing `.js` with HTML produces a syntax error in the console instead of
  // an honest 404, which is a genuinely confusing thing to debug.
...
const server = createServer((req, res) => {
  ...
  const record = lookup(req.url ?? '/');
  if (!record) {
    res.writeHead(404, { 'content-type': 'text/plain; charset=utf-8' }).end('Not found');
    return;
  }

  // The ETag identifies the resource, not the encoding, so this is checked
  // before picking one.
  ...
  const headers = {
    ...
    etag: record.etag,
    // Caches must key on the encoding, or a gzip response gets replayed to a
...
// Railway sends SIGTERM on redeploy. Closing cleanly lets in-flight requests
// finish instead of surfacing as connection resets during a rollout.
for (const signal of ['SIGTERM', 'SIGINT']) {
  process.on(signal, () => {
    console.log(`${signal} received, closing`);
    server.close(() => process.exit(0));
    // Don't let a hung keep-alive connection hold the deploy open forever.
