...
graph TB
    subgraph "Judgment Layer — Cursor Cloud Agents + Skills"
        ...
        USERBACK[userback-triage<br/>Daily]
    ...
    USERBACK -.->|Triage cards| SUPERTHREAD
    ...
    style USERBACK fill:#FFA500,stroke-dasharray: 5 5
    ...
    class CLUELESS,WORDFALL,CICHECK,MERGE,WIKI,PLAYEREMAIL,RELEASE built
    class CONDUCTOR,XGROWTH,EMAIL,HEALTH,EXPERIMENT,FEATUREDEV,QA,REVIEW,WIKIPEDIA,USERBACK,SECURITY,GROWTHSEO,ADCREATIVE planned
```
...
| **userback-triage** | *Cursor ad-hoc* | Daily | Superthread triage cards clustering feedback | n/a (triage only) | **[PARKED]** — Userback web widget exists (D-046); may become PostHog Surveys triage; schedule [PLANNED] |
...
**Job:** Cluster Userback feedback (or PostHog Surveys if Userback is removed) into Superthread triage cards. Daily.

**Trigger:** Daily Cursor Automation (editor setup [PLANNED]).

**Skill:** *Cursor ad-hoc.*

**Context:** Userback web widget exists (D-046), but CHANGELOG 0.8.21 says "Replaced Userback with PostHog Surveys" in a released version that is not in the tree. **Honest status: Userback code still exists in `src/userback/` as of this checkout.** If the widget is removed and PostHog Surveys replaces it, this bot triages PostHog survey responses instead. The bot reads feedback, clusters similar reports, and writes triage cards.
...
- **Status: [PARKED]** — no triage automation configured until feedback source is confirmed (Userback vs PostHog Surveys)

**Merge:** n/a — triage cards only. Owner or feature-dev bot acts on cards.

**Stop conditions:**
- Feedback source unavailable (Userback or PostHog credentials expired)
...
Each recurring bot (clueless-daily, wordfall-weekly, x-growth, player-email, product-health, experimentation, feature-dev, qa-playtest, code-review, wikipedia-reviewer, userback-triage, security-watch, growth-seo, ad-creative, wordkrush-conductor) must be configured in the Cursor Agents Window under Automations. The committed skill files (`.cursor/skills/clueless-daily-path/`, `wordfall-weekly-gauntlet/`) define the playbook; the Automation editor sets the schedule and branch.
