...
- [ ] **Wordfall trace gestures on desktop and iPad** (ST-89) — **Doing**; the board owns the pointer and scopes web selection/touch-action suppression to its tiles.
...
- [ ] **Cursor Daily Vault authoring Automation** — **Blocked on final editor setup**; the merge token is configured and proven. Configure the 18:00 GMT+1 automation in Cursor to update the standing `content/clueless-daily` PR and apply `automation:auto-merge`; the GitHub Action marks an eligible draft ready and merges only after CI passes for its current head (D-059, D-060).
