...
| 2026-08-25 | Wordfall weekly review path | Owner chose the standing `content/wordfall-weekly` review branch to maintain the four-Monday buffer without a Superthread card per level. Each row remains a reviewed, bundled, locally verified hard task. |
| 2026-08-25 | Content PR merge gate | Owner replaced the Clueless and Wordfall human merge queue with the `automation:auto-merge` label and an exact-head CI-gated GitHub Action (D-059). |
