...
| Result share | `src/games/share.ts`, `src/games/share-data.ts`, `src/games/<id>/share.ts`, `src/native/share.ts`, `src/native/share.native.ts`, `server/serve.mjs` `/share/:id`, `server/og-image.mjs` | [BUILT] — spoiler-free emoji grid per game; RN `Share` on native, `navigator.share` then clipboard on web; dynamic share URLs (`/share/:id`) carry encoded result data and serve 1200×630 PNG OG images via sharp (More or Less: green/red grid + streak; Clueless: heat spread + guess count; Wordfall: length-coded grid + score); never answers, guessed words, or item labels. Server handles HTML (strips all homepage `og:*`/`twitter:*`/`canonical` tags then injects per-result tags) and `/share/:id/og.png` generation. Invalid share IDs return HTTP 404, not the SPA homepage. `og-image.mjs` installs Fredoka SemiBold TTF via fontconfig at startup (`$TMPDIR/wordkrush-fonts/`) so librsvg/pango renders "Fredoka SemiBold" in SVG text elements; the grid emoji have no covering face and still render as `.notdef` boxes on Clueless and Wordfall (open, needs an owner call — see the note under this table). `og:image`/`twitter:image` carry `?v=<package version>` and the PNG is served `public, max-age=300`, so a render fix reaches scrapers that already unfurled a link; the image route matches on path alone, so any stamp resolves to the current render. `og:url` stays unstamped. Share paste carries the same emoji grid plus the dynamic `/share/:id` URL. OG description uses the standing share line. `server/serve.test.mjs` warms the real server over loopback and holds that contract. |
...
| Share-card grid emoji render as `.notdef` boxes | Medium | Only Fredoka is installed for librsvg, and it is a Latin display face with no emoji coverage, so Clueless's heat grid, Wordfall's length grid, and its `✓` verdict are boxes on the OG image while every Fredoka text tier renders correctly. Predates the cache work and is unchanged by it. Two dependency-free-ish fixes, both owner calls: bundle an emoji face for fontconfig (a new dependency, and colour emoji through librsvg is unreliable), or draw those grids as coloured `<rect>`s the way More or Less already does (a card-art change, so `docs/branding/`). The in-app and pasted emoji grids are unaffected — this is the server render only | Open |
