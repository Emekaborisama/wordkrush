...
The JSON diff goes up in a PR — **the diff is the content review**. CI (typecheck + tests) must pass ([ci.yml](../.github/workflows/ci.yml)). Merge → release ritual (WORKFLOW): changelog rollover, version bump, tag, `eas build` → `eas submit` → TestFlight → App Store. Web ships the same commit via `npm run build:web` → static host (O-6).
...
| Fairness guard + difficulty bands | `src/game/pairing.ts` (+ 9 tests) | [BUILT] |
| Engine reducer, pair selector, scoring | `src/game/engine.ts` *(to create)* | [PLANNED] |
...
- The app bundle contains no keys *structurally* — it reads bundled JSON only (D-007). Not a discipline rule; a property of the architecture.
- Future leaderboards will put limited Supabase access in the client: publishable key + Row Level Security, as its own logged decision.
