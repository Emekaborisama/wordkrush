...
| Result share | `src/games/share.ts`, `src/games/share-data.ts`, `src/games/<id>/share.ts`, `src/native/share.ts`, `src/native/share.native.ts`, `server/serve.mjs` `/share/:id`, `server/og-image.mjs` | [BUILT] — spoiler-free emoji grid per game; RN `Share` on native, `navigator.share` then clipboard on web; dynamic share URLs (`/share/:id`) carry encoded result data and serve 1200×630 OG images (More or Less: green/red grid + streak; Clueless: heat spread + guess count; Wordfall: length-coded grid + score); never answers, guessed words, or item labels. Server handles HTML with injected OG tags and `/share/:id/og.svg` generation. Share paste carries the same emoji grid plus the new dynamic URL. |
