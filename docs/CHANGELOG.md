...
### Added
- **Clueless Daily Vault levels 30–34: Measured Moment, Turning Point, Clean Slate, Clear Skies, and Fresh Start.** Five reviewed future solo levels extend the bundled Daily Vault buffer. None shows a hint; each theme stays sealed until players reach it. Native installs receive them with a later shipped build.
...
### Fixed
- **MORE and LESS on the share card now sit clear of X's title strip.** 0.8.35 made the composer unfurl a large card, and after a full minute LESS read cleanly while MORE sat under the green `WordKrush · More or Less` overlay. The stacked board is unchanged — two full-bleed photo cards above MORE / LESS, no title, grid, streak, names or values — but `server/og-image.mjs` now draws a `theme.bgElevated` bottom band under the button row (`MORE_OR_LESS_BOTTOM_SAFE_PAD`, 110px — the signed bar). The overlay lands on that pad; both buttons sit below a matching top-strip budget and above the pad. Photo slots shorten to match (still stacked, still `sharp.strategy.attention`). `server/og-image.test.mjs` asserts the button region is in that safe zone and that the pad itself is a flat dark band. Stamped `?v=0.8.36`. Share token encoding, photo ids, `/og/share/` path, truecolour encode, homepage lockup, Wordfall L4, analytics and races are untouched.

## [0.8.35] - 2026-09-04

### Fixed
- **A share link no longer ends on a character X's composer refuses to read.** Share tokens mapped base64's `=` padding to `~` for a cleaner-looking URL. `twitter-text` — the library X's own composer uses to find the links in a draft — allows `~` inside a URL but will not end one on it, so roughly half of all share links reached the composer truncated at the padding, taking the `?utm_source=player&utm_medium=share` query with them: the paste showed a bare blue link and built no card at all. `src/games/share-data.ts` now emits unpadded base64url — the standard `-` / `_` alphabet with the padding dropped, which is base64url's own answer to this — so a token is made only of characters a URL may end on. Nothing is lost from the links already in the wild: the decoder takes today's unpadded tokens, the `~`-padded tokens shared before this version, and either of those spellings with `=` padding, so a week-old paste and a composer-clipped copy of it both still resolve. 0.8.34's percent-decoding of `%7E` stays.
- **One share is now one board, whatever the link looked like when it arrived.** Pasting a `~`-token with its tildes removed by hand did unfurl, and drew the wrong card — different photos from the ones the player saw, because the pair was picked by hashing the encoded token and the padded and unpadded spellings hash differently. A More or Less share now names its two card photos in the payload (`photos`, from `src/games/more-or-less/card-photos.ts`), seeded by the standing and chosen at the moment the player shares. The server draws the pair the share named; links made before this version have no ids, so their pair is derived from the decoded payload rather than the token, which makes every spelling of one link agree. `server/og-card.mjs` is the new owner of that list and of the photo id ↔ Wikimedia source mapping; `src/games/more-or-less/card-photos.test.ts` fails if the client and server lists ever disagree.
- **The card X fetches now lives at a short static path.** `og:image` / `twitter:image` on a share page pointed at `/share/{token}/og.png` — the whole result payload, nested a level deep under a second dynamic route — while the homepage lockup X does unfurl is a flat `/og-image.png?v=…`. Cards are now served from `/og/share/{card}.png`, where the card id names only what the card draws: a More or Less board is `m_michael-jackson_albert-einstein`, at most 47 characters of path, because two photographs are the entire card. Clueless and Wordfall cards carry their own numbers, so their id is the result. Still a 1200×630 truecolour PNG (IHDR colour type 2), still stamped `?v=0.8.35` so no scraper is holding a cached copy of it, still `public, max-age=300`, and the path still resolves whatever the stamp says. `/share/{token}/og.png` keeps serving the identical bytes for anything that already cached that URL: the token resolves to the same card id, and the render cache is keyed on the card id rather than the share id, so one card is one render however it was reached.
- **A crafted share token can no longer reach the renderers.** The card renderers read their numbers straight out of the decoded payload — `Array(data.heatBuckets.cold)` is a grid square per guess — and a share token is whatever a stranger put in a URL, so a payload with a missing bucket threw inside the request and one with a nine-figure count asked this server to allocate an array of that size. Every route that can reach a renderer now validates the payload once, up front (`isDrawableShare`), and answers 404 to anything that is not a result. Photo ids a share names are only honoured when they match a photo we ship, so a stranger's path separators cannot reach the URL a share page declares.

### Deliberately unchanged
- The stacked board artwork: two full-bleed photo cards above MORE and LESS, on the game's own theme tokens, with no title, grid, streak, best, seen, item name, value or `? ? ?` — the only words on it are `More` and `Less`, and the standing still reads in the paste line above the link. The truecolour encode from 0.8.34, the dedicated `/share/:id` document from 0.8.33, and the homepage `og-image.png` lockup are all untouched.

## [0.8.34] - 2026-09-03

### Fixed
- **The More or Less share card is now a truecolour PNG, so X's composer can build a card from it.** 0.8.33 shipped the dedicated share document and the stacked board, and both halves answered a scraper correctly on Railway — `/share/:id` HTML 200 with no hub JSON-LD, the 1200×630 PNG 200 in ~0.6 s. Pasting a fresh share link into the X composer as @WordKrushGame still span for ~42 seconds and produced no card. Twitterbot was not the miss; the one property separating that card from the homepage lockup X does unfurl was the encode. The card was indexed — IHDR colour type 3, ~290–356 KB — because two photographs cost ~1 MB lossless and the quantiser took them to ~300 KB. `server/og-image.mjs` now emits colour type 2, 8-bit truecolour RGB: `palette: false` explicitly (sharp turns the quantiser back on for anyone who adds `quality`, `effort`, `colours` or `dither`), and `flatten` onto the card's own `theme.bg` so the rasteriser's RGBA becomes RGB rather than colour type 6 with an alpha channel of 255s. `adaptiveFiltering` is what carries the weight now the quantiser is gone — per-scanline filters take a two-photograph card from ~910 KB to ~400–770 KB, and `compressionLevel` stays at 6 because 9 buys under 5% for three times the CPU on a path a crawler waits on. Cards are still cached per share id, so one paste pays for one render. `server/og-image.test.mjs` and `server/serve.test.mjs` read IHDR off the encoded bytes — `sharp().metadata()` reports three channels for an indexed card as readily as a truecolour one, so colour type cannot be asked through a decoder — and require colour type 2 at bit depth 8 for all three games, with photos loaded and with a cold pool.
- **Everything 0.8.33 fixed stays fixed.** The card is the same stacked board: two full-bleed photo cards above MORE and LESS, no title, no grid, no streak, best or seen counters, no item names, values or `? ? ?`, and the only words on it are `More` and `Less`. `/share/:id` is still the dedicated OG document, not the hub with tags pattern-matched over it. Share-id `~` padding handling is unchanged. The homepage `og-image.png` lockup is untouched. `og:image` / `twitter:image` carry `?v=0.8.34`, so every share card moves onto a URL no scraper has cached — X keeps an unfurled card against its URL for up to seven days, so a fresh share link is the only honest way to confirm this.

## [0.8.33] - 2026-09-03

### Changed
- **A More or Less share card is now the board, not a scoreboard.** `/share/:id/og.png` for More or Less draws two stacked, full-bleed photo cards above the MORE and LESS buttons — the board as the phone plays it, tracing the signed crop — on `theme.bg`, with the game's own accent, tonal and hairline tokens. Gone: the title line, the green/red square grid, `Streak N` and `Best N`. The standing already reads in the paste line directly above the link, and repeating it as the whole picture made the card a stats blob — nothing about it said "this is a game you can play". The card carries no words but `More` and `Less`, so it also carries no item names, no values and no `? ? ?`; the pair that ended a run stays a spoiler, as it was before. `server/og-image.test.mjs` asserts both halves of that: the only text nodes on the card are `More` and `Less` and the render is identical for a run that scored 0 and one that scored 35, and the geometry is stacked — one left edge, second card below the first, full-bleed, buttons under both. Clueless and Wordfall still draw their result grid unchanged.
- **A card photo is cropped for salience, because the slot is a sliver.** Stacking two full-width rows inside 1200×630 makes each photo slot 4.8:1, so which band of a mostly-portrait source survives decides whether the photo is recognisable at all. `sharp.strategy.attention` picks the most salient band, which on a Wikipedia lead portrait is the face. A fixed band does not work: cropping from the top leaves hair and background, and it also collapses the entropy separation the licence filter depends on — the Titanic and a blue whale fall to 4.2 and 4.9, below an app screenshot, so photographs stop being distinguishable from flat graphics at all.
- **Card photos are fetched before the port opens, not on the request that needs them.** `server/og-photos.mjs` fills a pool from the bundled category data during boot, alongside the pre-warming `server/serve.mjs` already did for the bundle, and a card render only reads memory. A scraper fetches a card once and renders whatever came back, so a Wikimedia round trip inside that request is the difference between a card and a blank one. Only images whose licence needs no credit (public domain, PD, CC0) are eligible, because this card shows no text and CC BY / CC BY-SA oblige us to display one; `GameScreen` still credits every photo it puts on screen. A minimum image entropy of 6 keeps photographs and drops the flat vectors Wikipedia leads with for a country or a brand — measured at this slot size, flags and logos land between 0.8 and 2.2, app screenshots at 3.4 and 5.6, and photographs between 6.1 and 7.6. A pair is picked from the share id in dataset order, so one link renders one board across restarts, and a cold pool still draws the board.
- **Rendered cards are cached in memory, and no longer claim to vary.** A card is a rasterise plus a palettised PNG encode; one paste brings a burst of scrapers to a single id, so the first pays ~150 ms and the rest read memory (bounded at 256 ids, oldest out). The palette encode also takes the two-photograph card from ~1 MB to ~310 KB, a fraction of what X keeps after re-encoding anyway. The image response no longer sends `Vary: Accept-Encoding`: it is one PNG served uncompressed, and nothing about it varies on the request.

### Fixed
- **The LESS button no longer renders with a dark chevron blotted across it.** The palette quantiser spends its 256 entries on the two photographs, which leaves the tonal button's translucent fill to be approximated — and dithered, that approximation is not noise but large structured shapes, reading as a rendering fault on one of the two elements the card exists to show. `dither: 0` rounds a flat fill to a single palette entry and keeps it flat, while the photographs are dense enough that no banding appears at card size. `server/og-image.test.mjs` samples inside each button, clear of the rounded corners, the arrow and the label, and requires zero variance; with dithering restored that sample measures 5.9 and the test fails.
- **`/share/:id` is now a document about one result, and nothing else.** It was the hub document with per-result `og:*` tags pattern-matched over the top, which left the hub's `<meta name="description">`, its JSON-LD (`url: https://wordkrush.com/`, describing the whole site) and its "Choose your game" copy sitting underneath the result's tags — one page making two different claims, so anything that resolved them in the wrong order described the WordKrush homepage and anything that refused to guess showed nothing. `scripts/patch-web-head.mjs` now delimits every part of the built shell that describes the hub (`HEAD_MARKERS`, `COPY_MARKERS`), and `server/share-document.mjs` replaces those blocks whole. Nothing about the homepage can survive, because nothing is matched. The result's own head carries `description`, a self `canonical`, `og:image:type`, `og:image:alt` / `twitter:image:alt` and `twitter:site`, and the crawler-readable copy in `#root` and `<noscript>` describes the result — both copies, where a single replace previously left the second one on the homepage's story. The playable bundle and the layout CSS are untouched, so a player who follows a share link still lands in the game.
- **A share id whose `~` arrived percent-encoded no longer 404s.** The share routes read `url.pathname`, which keeps its percent-escapes, while static lookup decoded first — so `%7E` (a legal spelling of `~`, which is unreserved) asked for an id the server had never heard of and got a 404, for the card image as readily as for the page. Ids are decoded to one canonical form before decoding the payload, and the URLs a share page declares use that form whatever spelling was requested. Roughly half of all More or Less payloads produce an id with `~` padding.
- **A share id that lost its padding in a composer still resolves.** `twitter-text`, the library X's own composer uses to find links in a draft, allows `~` inside a URL but never at the end of one, so a padded id reaches a scraper with its tail — and the whole `?utm_source=player&utm_medium=share` query behind it — cut off. Base64 padding is now restored rather than assumed. The share id scheme itself is unchanged; `bestgames.*` storage keys are untouched.
- **A shell without the share markers fails at boot.** `warm()` refuses an `index.html` that `scripts/patch-web-head.mjs` no longer marks, rather than letting every share page quietly go back to serving the homepage's description. The layout CSS moved from after `</title>` to after the marked head block for the same reason: the title lives inside that block, so the old anchor would have let a share page sweep the web build's layout out with the hub's tags.

## [0.8.32] - 2026-09-02

### Changed
- **Analytics is now default on, with an opt-out toggle.** The blocking consent modal on first launch has been removed, reducing friction. Users can now turn analytics off (or on) at any time from the Account screen.
- **$pageview allowlisted for web.** PostHog web analytics now correctly captures `$pageview` events to track visitor counts, which was previously blocked by the strict event allowlist.
