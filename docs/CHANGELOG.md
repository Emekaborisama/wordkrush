...
### Changed
- **Judgment work is a named Grok-bot fleet catalogued in docs/AGENT-OS.md.** Cursor Cloud Agents plus `.cursor/skills/` own content authoring, organic X growth, QA, and product health. GitHub Actions still run CI, Railway deploy, GitHub Release, Wikipedia rotate, Resend send, and CI-gated content merge. A bot does not replace those jobs and does not push to master. Clueless and Wordfall bots (skills committed, Grok schedules live at 8:55 daily and Monday 10:55 London, Agents Window owner-configured) use `.cursor/skills/clueless-daily-path/` and `.cursor/skills/wordfall-weekly-gauntlet/`. Conductor (read-only daily pulse, Grok weekdays 9:55, skill [PLANNED]), WordKrush QA, Product Health, WordKrush Dev, and WordKrush X (organic X for @WordKrushGame, `.claude/skills/wordkrush-x-growth/`, Cursor port [PLANNED]) are catalogued.
