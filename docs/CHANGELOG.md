...
### Changed
- **A More or Less share card is now the board, not a scoreboard.** `/share/:id/og.png` for More or Less draws two photo cards side by side above the MORE and LESS buttons, on `theme.bg`, with the game's own accent, tonal and hairline tokens. Gone: the title line, the green/red square grid, `Streak N` and `Best N`. The standing already reads in the paste line directly above the link, and repeating it as the whole picture made the card a stats blob — nothing about it said "this is a game you can play". The card carries no words but `More` and `Less`, so it also carries no item names, no values and no `? ? ?`; the pair that ended a run stays a spoiler, as it was before. `server/og-image.test.mjs` asserts that literally — the only text nodes on the card are `More` and `Less`, and the render is byte-identical for a run that scored 0 and a run that scored 35. Clueless and Wordfall still draw their result grid unchanged.
- **The card runs side by side rather than stacked as the phone does.** Two full-width rows inside 1200×630 are 4.8:1 slots, and a Wikipedia lead image cropped that flat is a necktie or a slice of a flag — the photos stopped being recognisable, which is the one thing this card exists to show. Turning the pair sideways gives each photo a near-square slot, keeps the prompt the game actually asks, and lands every element inside the 2:1 band X centre-crops a `summary_large_image` to.
- **Card photos are fetched before the port opens, not on the request that needs them.** `server/og-photos.mjs` fills a pool from the bundled category data during boot, alongside the pre-warming `server/serve.mjs` already did for the bundle, and a card render only reads memory. A scraper fetches a card once and renders whatever came back, so a Wikimedia round trip inside that request is the difference between a card and a blank one. Only images whose licence needs no credit (public domain, PD, CC0) are eligible, because this card shows no text and CC BY / CC BY-SA oblige us to display one; `GameScreen` still credits every photo it puts on screen. A minimum image entropy of 6 keeps photographs and drops the flat vectors Wikipedia leads with for a country or a brand — measured over that set, flags and logos land between 0.6 and 2.1, app screenshots between 4.5 and 5.4, and photographs between 6.4 and 7.7. A pair is picked from the share id in dataset order, so one link renders one board across restarts, and a cold pool still draws the board.
- **Rendered cards are cached in memory, and no longer claim to vary.** A card is a rasterise plus a palettised PNG encode; one paste brings a burst of scrapers to a single id, so the first pays ~200 ms and the rest read memory (bounded at 256 ids, oldest out). The palette encode also takes the two-photograph card from ~890 KB to ~310 KB with no visible banding at card size, which is a third of what X keeps after re-encoding anyway. The image response no longer sends `Vary: Accept-Encoding`: it is one PNG served uncompressed, and nothing about it varies on the request.
