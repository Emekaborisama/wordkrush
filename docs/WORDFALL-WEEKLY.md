...
Signed-in players are mailed on **Tuesday 09:00 UTC** (`pipeline/player-email.ts`, D-054) about that Monday’s drop **and** any player-facing changelog from the last 7 days. The email does not fetch levels; it reads the same bundled catalog. No dated row and no player-facing changelog this week → the job skips.
